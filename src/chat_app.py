...
async def get_cached_image_description(image_url: str, image_cache: dict) -> str:
    ...
    if image_url in image_cache:
        logger.debug("Using cached image description", extra={
                     "url": image_url[:50], "cache_size": len(image_cache)})
        return image_cache[image_url]

    logger.debug("Fetching new image description",
                 extra={"url": image_url[:50]})
    try:
        ...
        logger.error("Failed to get image description", extra={
                     "url": image_url[:50], "error": str(e)})
        return ""
...
async def pre_fetch_image_description(image_url: str, image_cache: dict):
    ...
    if image_url and image_url not in image_cache:
        logger.debug("Pre-fetching image description",
                     extra={"url": image_url[:50]})
        try:
            loop = asyncio.get_event_loop()
            description = await loop.run_in_executor(thread_pool, get_image_description, image_url)
            image_cache[image_url] = description
            logger.debug("Pre-fetched and cached image description",
                         extra={"url": image_url[:50]})
        except Exception as e:
            logger.error("Failed to pre-fetch image description",
                         extra={"url": image_url[:50], "error": str(e)})
...
def select_agent(handoff_reply: str, env_vars: Dict[str, str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    log_timing("Agent Selection", start_time,
               f"Selected: {result[1] if result[1] else 'None'}")
    return result
...
def call_handoff(handoff_client: ChatCompletionsClient, handoff_prompt: str, formatted_history: str, phi_4_deployment: str) -> str:
    ...
    with tracer.start_as_current_span("custom_function") as span:
        ...
        try:
            ...
            log_timing("Handoff Call", start_time,
                       f"Model: {phi_4_deployment}")
            return result
        except Exception as e:
            ...
            if "content_filter" in err_str or "ResponsibleAIPolicyViolation" in err_str:
                ...
                log_timing("Handoff Call (Content Filter Error)",
                           start_time, f"Error: {err_str[:50]}...")
                return result
            # Otherwise, re-raise
            log_timing("Handoff Call (Exception)", start_time,
...
if not project_endpoint:
    raise ValueError(
        "AZURE_AI_AGENT_ENDPOINT environment variable is required")
project_client = AIProjectClient(
    endpoint=project_endpoint,
    credential=DefaultAzureCredential(),
)

HANDOFF_PROMPT_PATH = os.path.join(os.path.dirname(
    os.path.abspath(__file__)), 'prompts', 'handoffPrompt.txt')
FALLBACK_PROMPT_PATH = os.path.join(os.path.dirname(
    os.path.abspath(__file__)), 'prompts', 'fallBackPrompt.txt')
CORA_FALLBACK_PROMPT_PATH = os.path.join(os.path.dirname(
    os.path.abspath(__file__)), 'prompts', 'CoraPrompt.txt')
CART_UPDATE_PROMPT_PATH = os.path.join(os.path.dirname(
    os.path.abspath(__file__)), 'prompts', 'addToCartPrompt.txt')
...
@app.get("/")
async def get():
    chat_html_path = os.path.join(os.path.dirname(
        ...
        with tracer.start_as_current_span("Run Customer Loyalty Thread"):
            ...
            if not customer_loyalty_id:
                session_loyalty_response = {
                    "answer": "Customer loyalty agent not configured.", "agent": "customer_loyalty"}
                log_timing("Customer Loyalty Task",
                           start_time, "Agent not configured")
                return
            ...
            parsed_response["agent"] = "customer_loyalty"
            ...
            log_timing("Customer Loyalty Task", start_time,
                       f"Discount: {session_discount_percentage}")
        ...
        while True:
            ...
            try:
                ...
                if image_url:
                    ...
                    logger.debug("Persistent image URL updated",
                                 extra={"url": persistent_image_url})
                    log_cache_status(image_cache, image_url)
                    # Pre-fetch the image description asynchronously
                    asyncio.create_task(
                        pre_fetch_image_description(image_url, image_cache))

                # Append user message to raw_io_history
                raw_io_history.append(
                    {"input": user_message, "cart": persistent_cart})
                log_timing("Message Parsing", message_start_time,
                           f"Message length: {len(user_message)} chars")
            except WebSocketDisconnect:
                logger.info(
                    "WebSocket connection terminated - client disconnected from endpoint")
                break
            except Exception as e:
            ...
            try:
                if conversation_history:
                    ...
                    chat_history.append(("user", user_message))
                log_timing("History Parsing", history_start_time,
                           f"History entries: {len(chat_history)}")
            except Exception as e:
                logger.error(
                    "Error parsing conversation history", exc_info=True)
                chat_history.append(("user", user_message))
...
#            except Exception as e:
#                logger.error(
#                    "Error during single-agent response generation", exc_info=True)
#                await websocket.send_text(fast_json_dumps({"answer": "Error during single-agent response generation", "error": str(e), "cart": persistent_cart}))
            ...
            try:
                ...
                logger.debug(
                    "Handoff agent execution initiated - commencing agent selection protocol")
                ...
                logger.debug(
                    "Handoff agent response received - agent selection criteria processed")
                logger.debug(f"Handoff reply: {handoff_reply}")
                log_timing("Handoff Processing", handoff_start_time,
                           f"Reply length: {len(handoff_reply)} chars")
                ...
                if isinstance(handoff_reply, str) and handoff_reply.startswith("__CONTENT_FILTER_ERROR__"):
                    error_message = handoff_reply.replace(
                        "__CONTENT_FILTER_ERROR__", "").strip()
            ...
            try:
                if 'cart' in user_message.lower():
                    ...
                    cart_prompt = CART_UPDATE_PROMPT + "\nRAW_IO_HISTORY:\n" + \
                        fast_json_dumps(list(raw_io_history),
                                        option=orjson.OPT_INDENT_2)
                    logger.debug(
                        "Cora agent cart update operation initiated - commencing cart state modification")
                    ...
                    try:
                        ...
                        logger.error(
                            "Error processing cart/cora", exc_info=True)
                    ...
                    logger.debug(
                        "Cart update operation completed - cart state successfully modified")
                    logger.debug(
                        "Cora agent thread execution terminated - response processing complete")

                    # Use new merge_cart_and_cora utility for robust merging
                    merged = merge_cart_and_cora(
                        cart_reply_raw, cora_reply_raw)
                    ...
                    response_json = fast_json_dumps(
                        {**merged, "cart": persistent_cart})
                    raw_io_history.append(
                        {"output": response_json, "cart": persistent_cart})
                    ...
                    log_timing("Cart/Cora Processing", cart_start_time,
                               f"Cart items: {len(persistent_cart)}")
                    ...
                    if session_loyalty_response and not loyalty_response_sent:
                        loyalty_response_with_cart = {
                            **session_loyalty_response, "cart": persistent_cart}
            ...
            try:
                ...
                agent_selected, agent_name = select_agent(
                    handoff_reply, validated_env_vars)
                if not agent_selected or not agent_name:
                    ...
                    continue
                logger.debug(
                    f"Agent selection protocol completed - {agent_name} agent designated for task execution")
                log_timing("Agent Selection", agent_selection_start_time,
            ...
            try:
                ...
                if agent_name == "interior_designer":
                    logger.debug(
                        "Interior Designer agent execution initiated - commencing design consultation protocol")
                    with tracer.start_as_current_span("Zava Interior Designer Agent Call"):
                        ...
                        if not image_url and not video_url:
                            ...
                            log_timing("Product Recommendations", product_start_time,
                                       f"Products found: {len(products) if products else 0}")
                            logger.debug(
                                "Product recommendation engine execution completed - catalog query processed")
                            ...
                            log_timing("Interior Designer Fallback",
                                       fallback_start_time, "No image/video")
                            ...
                            if image_url:
                                ...
                                log_timing(
                                    "Image Analysis", image_start_time, f"URL: {image_url[:50]}...")
                                multimodal_data = image_data
                                analysis_msg = get_rotating_message(
                                ...
                                logger.debug(
                                    "Image analysis pipeline completed - visual content processing terminated")

                                product_start_time = time.time()
                                products = product_recommendations(
                                    user_message + multimodal_data + "paint accessories, paint sprayers, drop cloths, painters tape")
                                log_timing("Product Recommendations", product_start_time,
                                           f"Products found: {len(products) if products else 0}")
                                logger.debug(
                                    "Product recommendation engine execution completed - catalog query processed")
                                ...
                                log_timing("Interior Designer Fallback",
                                           fallback_start_time, "With image")
                                msg = fallback_reply
                                bot_reply = extract_bot_reply(msg)
                                ...
                                logger.debug(
                                    "Video analysis initiated - commencing video content processing")
                                video_summary = get_video_summary(video_url)
                                thank_you_msg = get_rotating_message(
                                ...
                                log_timing(
                                    "Video Analysis", video_start_time, f"URL: {video_url[:50]}...")
                                multimodal_data = video_summary
                                analysis_msg = get_rotating_message(
                                    VIDEO_ANALYSIS_MESSAGES)
                                await websocket.send_text(fast_json_dumps({"answer": analysis_msg, "agent": "interior_designer", "cart": persistent_cart}))
                                logger.debug(
                                    "Video analysis pipeline completed - temporal content processing terminated")
                                ...
                                fallback_prompt = "Received video from user:" + \
                                    FALLBACK_PROMPT + f"\n\n {user_message}"
                                ...
                                log_timing("Interior Designer Fallback",
                                           fallback_start_time, "With video")
                                msg = fallback_reply
                                bot_reply = extract_bot_reply(msg)
                    ...
                    with tracer.start_as_current_span("Zava Interior Designer Agent Call"):
                        ...
                        log_timing("Image Analysis (Create)", image_start_time,
                                   f"URL: {persistent_image_url[:50]}...")
                        multimodal_data = image_data
                        logger.debug(
                            "Image analysis pipeline completed - visual content processing terminated")
                        ...
                        product_start_time = time.time()
                        products = product_recommendations(
                            user_message + "paint accessories, sprayers, drop cloths, painters tape")
                        log_timing("Product Recommendations", product_start_time,
                                   f"Products found: {len(products) if products else 0}")
                        logger.debug(
                            "Product recommendation engine execution completed - catalog query processed")
                        ...
                        image = create_image(
                            text=user_message, image_url=persistent_image_url)
                        ...
                        product_names = extract_product_names_from_response(
                            response_data)
                        chat_history.append(
                            ("bot", bot_answer + product_names))

                        await websocket.send_text(response_json)
                        log_timing(
                            "Agent Execution", agent_execution_start_time, f"Agent: {agent_name}")
                        continue  # Skip the common response handling below

                elif agent_name == "cora":
                    logger.debug(
                        "Cora agent execution initiated - commencing conversational AI protocol")
                    with tracer.start_as_current_span("Agent Cora Call"):
                        ...
                        log_timing("Cora Agent Call",
                                   cora_start_time, "Fallback model")
                        msg = cora_fallback_reply
                        bot_reply = extract_bot_reply(msg)
                    logger.debug(
                        "Cora agent execution terminated - conversational AI protocol completed")

                else:
                    logger.debug(
                        f"{agent_name} agent execution initiated - commencing specialized task protocol")
                    ...
                    logger.debug(
                        f"{agent_name} agent execution terminated - specialized task protocol completed")
                    bot_reply = ""
                    async for msg in processor.run_conversation_with_text_stream(input_message=user_message):
                        bot_reply = extract_bot_reply(msg)

                log_timing("Agent Execution",
                           agent_execution_start_time, f"Agent: {agent_name}")
                ...
                response_json = fast_json_dumps(
                    {**parsed_response, "cart": persistent_cart})
                raw_io_history.append(
                    {"output": response_json, "cart": persistent_cart})
                await websocket.send_text(response_json)
            except Exception as e:
        ...
        logger.info(
            f"WebSocket Session Ended - Duration: {session_duration:.3f}s")
